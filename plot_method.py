...
class PlotMethod(object):
    ...
    def __plot_urata_servo(plot_item, times, data_dict, logs, log_cols, cur_col, key, i, offset1, offset2=1):
        plot_item.plot(times, data_dict[logs[0]][:, (PlotMethod.urata_len+1) * log_cols[0] + (offset1+offset2)], linewidth=2, label=key)
    ...
    def plot_abs_enc(plot_item, times, data_dict, logs, log_cols, cur_col, key, i):
        plot_item.plot(times, [math.degrees(x) for x in data_dict[logs[0]][:, (PlotMethod.urata_len+1) * log_cols[0] + (6+1)]], linewidth=2, label='abs - enc')

    @staticmethod
    def plot_rh_q_st_q(plot_item, times, data_dict, logs, log_cols, cur_col, key, i):
        plot_item.plot(times, [math.degrees(x) for x in (data_dict[logs[1]][:, log_cols[1]] - data_dict[logs[0]][:, log_cols[0]])], 'r', linewidth=2, label=logs[1]+" - rh_q")
    ...
    def plot_rad2deg(plot_item, times, data_dict, logs, log_cols, cur_col, key, i):
        ...
        plot_item.plot(times, data_deg, linewidth=2, label=key)
    ...
    def plot_watt(plot_item, times, data_dict, logs, log_cols, cur_col, key, i):
        ...
        plot_item.plot(times, watt, linewidth=2,  label=logs[0])

    @staticmethod
    def plot_comp(plot_item, times, data_dict, logs, log_cols, cur_col, key, i):
        plot_item.plot(times, data_dict[logs[0]][:, log_cols[0]], linewidth=1+len(logs)-i, label=logs[0])
    ...
    def plot_COP(plot_item, times, data_dict, logs, log_cols, cur_col, key, i):
        ...
        plot_item.plot(times, -tau_y/f_z, linewidth=2, label=key+"x")
        plot_item.plot(times,  tau_x/f_z, linewidth=2, label=key+"y")

    @staticmethod
    def normal(plot_item, times, data_dict, logs, log_cols, cur_col, key, i):
        plot_item.plot(times, data_dict[logs[0]][:, log_cols[0]], linewidth=2, label=key)
